...
func (jm *Controller) getOrCreateWorkload(ctx context.Context, job *batch.Job, pods []*v1.Pod) (*schedulingv1alpha2.Workload, error) {
	...
	err := retry.OnError(retry.DefaultRetry, apierrors.IsAlreadyExists, func() error {
		...
		case 1:
			...
			return err
		default:
			utilruntime.HandleErrorWithLogger(logger, nil, "multiple Workloads referencing Job falling back to default scheduling", "workloadsCount", len(matched), "job", job.Name, "namespace", job.Namespace)
			return nil
		}
	})
	return result, err
...
func (jm *Controller) getOrCreatePodGroup(ctx context.Context, job *batch.Job, workload *schedulingv1alpha2.Workload) (*schedulingv1alpha2.PodGroup, error) {
	...
	err := retry.OnError(retry.DefaultRetry, apierrors.IsAlreadyExists, func() error {
		...
		case 1:
			...
			return nil
		default:
			utilruntime.HandleErrorWithLogger(logger, nil, "multiple PodGroups referencing Workload falling back to default scheduling", "PodGroupsCount", len(matched), "workload", workload.Name, "namespace", workload.Namespace)
			return nil
		}
	})
	return result, err
